...
JumpEmblem.prototype.type="JumpEmblem";
...
JumpEmblem.prototype.act=function(step){
    this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
var playerXSpeed = 7;
var playerXSlimeSpeed= 3;


Player.prototype.moveX = function(step, level, keys) {
  this.speed.x = 0;
  if (keys.left){
  if(this.debuff=="slime"){
    this.speed.x-=playerXSlimeSpeed;
  }
  else{
...
var tempPlayer = this.actors.find(function(actor){return actor.type=="player";})
  // if the player touches  and the player hasn't won
  // Player loses
  if (type == "lava" && this.status == null) {
    ...
    tempPlayer.debuff="slime";
    
  }
  
  else if(type=="JumpEmblem"){
    tempPlayer.poweredUp=true;
    
        setTimeout(function(){powerDown(tempPlayer);},3000);
